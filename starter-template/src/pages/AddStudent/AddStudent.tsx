...
export default function AddStudent() {
  ...
  console.log(addStudentMutate.error)
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    addStudentMutate.mutateAsync(formData)
    console.log(formData)
  }

  const errorForm = useMemo(() => {
    if (
      isAxiosError<{ error: FormError }>(addStudentMutate.error) &&
      addStudentMutate.error.response?.status === 422
    ) {
      return addStudentMutate.error.response?.data?.error
    }
    return null
